...
from deep_research_agent.models import SearchResult
from deep_research_agent.research_agents.follow_up_agent import FollowUpDecisionResponse, follow_up_decision_agent
from deep_research_agent.research_agents.search_agent import search_agent
from deep_research_agent.research_agents.query_agents import QueryResponse, query_agent
from deep_research_agent.research_agents.synthesis_agent import synthesis_agent
...
class ResearchCoordinator:
    ...
    async def perform_research_for_queries(self, queries: list[str]) -> None:
        ...
        all_search_results: dict[str, list[dict[str, str]]] = {}
    ...
    async def synthesis_report(self) -> str:
        with console.status("[bold cyan]Synthesizing research findings...[/bold cyan]") as status:
            ...
            synthesis_result = await Runner.run(synthesis_agent, input=findings_text)
    ...
    async def generate_followup(self) -> FollowUpDecisionResponse:
        with console.status("[bold cyan]Evaluating if more research is needed...[/bold cyan]") as status:
            ...
            follow_up_result = await Runner.run(follow_up_decision_agent, input=findings_text)
            decision = follow_up_result.final_output

            console.print(Panel(f"[bold cyan]Follow-up Decision[/bold cyan]"))
            console.print(f"[yellow]Decision:[/yellow] {'More research needed' if decision.should_follow_up else 'Research complete'}")
            console.print(f"[yellow]Reasoning:[/yellow] {decision.reasoning}")

            if decision.should_follow_up:
                console.print("\n[yellow]Follow-up Queries:[/yellow]")
                for i, query in enumerate(decision.queries, 1):
                    console.print(f"  {i}. {query}")
